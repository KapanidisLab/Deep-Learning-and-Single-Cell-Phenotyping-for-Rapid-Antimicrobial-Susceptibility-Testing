...
def TrainTestVal_split(**kwargs):
    '''
        ...
        ----------
        data_folder: string
            path to directory with image data. Data should be sorted and combined.
    ...
    annotation_folder = kwargs.get('annotation_folder', False)
    output_folder = kwargs.get('output_folder', False)
    proportions = kwargs.get('proportions', False)
    seed = kwargs.get('seed', False)

    if not all([data_folder, annotation_folder, output_folder, proportions, seed]):  # Verify input
        raise TypeError
    ...
    for folder in seg_folders:
        for image in os.listdir(data_folder):
            if fnmatch.fnmatch(image, folder+'.*'):
                segs = os.path.join(annotation_folder,folder)
                img = os.path.join(data_folder,image)
                matches.append([segs,img])
