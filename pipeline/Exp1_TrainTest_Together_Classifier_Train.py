...
# ---PREPARE DATASET WITH BOTH CHANNELS
    ...
    pipeline.FileOp('TrainTestVal_split', data_sources = [files_WT,files_RIF], annotation_sources = [annots_WT,annots_RIF], output_folder=output,test_size = 0.2, validation_size=0.2, seed=42 )
    ...
    train(mode='DenseNet121', X_train=X_train, y_train=y_train, size_target=resize_target, class_count=class_count, pad_cells=True,
          logdir=logdir, batch_size=64, epochs=100,learning_rate=0.0005, optimizer='NAdam', verbose=True, dt_string='DenseNet121')
